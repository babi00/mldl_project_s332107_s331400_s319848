...
def main():
    ...
    model_3 = PPO("MlpPolicy", source_env, learning_rate= best_hp['lr'], n_steps=best_hp['nsteps'], gamma=best_hp['gamma'], batch_size=best_hp['batch_size'], n_epochs=best_hp['n_epochs'], gae_lambda=best_hp['gl'], seed=42, verbose=2, tensorboard_log=f'runs/{run.id}')
    model_3.learn(total_timesteps=1_000_000, callback = WandbCallback(model_save_path = f'models/{run.id}', verbose=2)) # visualize last model trainig curve with wandb callback
