""" Implementation of Automatic Domain Randomization, training a model on three different seeds (0,14,42) for 10M timesteps 
in order for the results to be statistically more significant. """
...
class AutoDRCallback(BaseCallback):
    ...
    def _on_training_start(self) -> None: # before the first rollout
        ...
        #step-size, a percentage of the original masses
        self.deltas = {
        ...
        wandb.init(project="auto_dr_testing_2", config={
            "delta": self.deltas,
...
def main():
    ...
    #after tuning, we selected the best to run for 10M
    delta_values = [0.02] 
    ...
    for delta in delta_values:
        for threshold_high in threshold_high_values:
            ...
            model_1.save('./models/model_1_autoDR')
            ...
            run_general = wandb.init(project="auto_dr_testing_2", name=run_name_general, config={
                "delta": delta,
